...
def parse_species_prefix_and_mangled(header: str, symbol: str) -> Tuple[str, str]:
    """
    Parse Orthocaller protein headers for CDS lookup.

    species_key = text before the FIRST underscore `_`

    mangled = ID-like text after the first underscore.
      - If a gene symbol was recovered from the filename, stop before that symbol.
      - If no gene symbol exists in the filename, keep the rest of the header so
        rebuilt_candidates_from_mangled() can recover ENS/XP/NP/FUN/etc IDs.

    This makes the script tolerate filenames like:
      1000_generax-Gene-33__OG0005939_Dclean_...faa
    where no `__geneSymbol__` field is present.
    """
    ...
    if symbol:
        ...
        if pos != -1:
            return species_key, rest[:pos].strip()

    # Symbol-less fallback: retain ID-bearing portion of the header.
    return species_key, rest
...
def process_one_file(
    ...
    if not symbol:
        # Older version hard-failed here with `missing_symbol_in_filename`.
        # For symbol-less filenames, keep going and parse IDs directly from headers.
        symbol = ""
        if debug:
            print(f"[WARN] No gene symbol in filename; using header-based ID parsing: {proteins_path.name}")
